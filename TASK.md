...
3. **匹配成功率低** —— **少爷的截图改写了诊断**：失败行写的是「查询失败」（= 请求抛异常），不是「未找到」；
   ...
   **诚实的边界**：纯 VR/独占（Zenith、Vegas Infinite、Epic Roller Coasters 这类）TapTap 没有、
   索尼也没给中文名 ⇒ **只有 IGDB 密钥能救**（2026-09-23 少爷已配好密钥并确认可用 ⇒ B5 闭环，见 D30）；B5 已更新为"别名已做 + 请少爷配 IGDB"。
...
### D30 · IGDB 密钥就位与 cn-v0.44.3 三端发版（2026-09-23）

**B5 的最后一环落地**：少爷在 Twitch 开发者后台完成 IGDB 授权，密钥已配置并确认可用。此前卡住
这条的从来不是密钥本身，而是**授权域 `id.twitch.tv` 在大陆被墙**（数据域 `api.igdb.com` 可裸连）
叠加**授权调用曾与数据调用共用 5 秒超时**（D25 续已把授权单独放宽到 30 秒）。令牌为
`client_credentials` 类型、约 **60 天**有效 ⇒ 实际使用形态是「**两月借一次网续令牌**」，续完之后
数据请求全程直连。纯 VR／主机独占标题（Zenith、Vegas Infinite 这类，TapTap 无中文名、索尼也不给
本地化名）自此由 IGDB 目录接管。

**D28 图标风格定案**：少爷拍板「有图即可，直接用」—— 六个源保持厂商原始形态（多为**实心方底**），
