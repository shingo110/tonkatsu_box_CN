...
| B5 | **PSN 导入的匹配率低**（D24 少爷真机反馈） | 病因已定位：中文游戏名走 **TapTap（手游目录）**，PS5 的 3A 大作大多不在其中；纯拉丁名走 **IGDB**，而**少爷确认尚未配 IGDB 密钥** ⇒ 英文名实际上**没有可用目录**（只剩 TapTap 兜底）。**少爷已拍板：做「别名匹配」**（②）；③ PSN 自建目录（要加 `DataSource`，工作量等同 D16–D19 加源）押后 | **下一轮开工**（等少爷的预览页截图：若 `name` 与 `localizedName` 实际相同，别名就是空转，须另找病根） |
