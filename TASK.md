...
3. **匹配成功率低** —— 已定位病因与可选处方，**待少爷拍板**（见「阻塞与长期债」）。

**四关**：analyze 干净 · RPC 字节一致 · 应用 **5906 通过 / 3 跳过** · core **2385** · server **116**
（较 D23：应用 +20、服务端 +1）。
...
| B5 | **PSN 导入的匹配率低**（D24 少爷真机反馈） | 病因已定位：中文游戏名走 **TapTap（手游目录）**，PS5 的 3A 大作大多不在其中；纯拉丁名走 **IGDB**，而那需要少爷自配密钥。**两条腿一条瘸一条未必接上**。可选处方：① 配 IGDB 密钥（覆盖主机游戏最全，但库里中文名搜不了英文目录）；② **别名匹配** —— REST 游玩历史同时有 `name` + `localizedName`，两个名字分别喂两个目录并合并候选，**与少爷的配置无关地提升命中**；③ 让 PSN 自己成为目录（要加 `DataSource`，工作量等同 D16–D19 加源） | **待少爷拍板**（②是纯增益、不动阈值，RULES 七之十八 的纪律仍须遵守） |
