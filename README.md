...
>
> 本分支已发布三端构建产物，见 [Releases](https://github.com/shingo110/tonkatsu_box_CN/releases)。发布流水线（`.github/workflows/release-cn.yml`，推 `cn-v*` 标签触发）一次打包 Windows / Android / 自托管 Web 三端，推一个标签即出全套。
...
| 🎨 | **画板** | 拖拽式画布，可放海报、便签与连线 |
| 🖋 | **界面字体** | 从本机已安装的字体中挑选界面字体（桌面端） |
| 🎛 | **界面主题** | 深色 / 樱花 / PS1 复古灰三套配色，随选随生效 |
...
| 类型 | 目录服务 | 密钥 | 国内直连 |
|------|---------|------|:--------:|
| 游戏 | [IGDB](https://www.igdb.com/) | 内置 | ⚠️ 需出境 |
...
> [!NOTE]
> **关于短剧**：短剧在分类上属于「剧集」。有豆瓣词条的短剧（如平台爆款）会和长剧一同出现在剧集搜索结果中；但海量小程序独播短剧（抖音 / 快手等）若没有豆瓣条目则搜不到 —— 这是数据缺口，不是代码缺口。

## 网络与代理（与原版的使用差异）

上游在海外网络下设计，不关心代理；本分支面向大陆网络，部分数据源需要出境，因此比原版多了一层网络设定。**核心事实**：Dart 的 `HttpClient` 不会读取系统或 VPN 代理，应用的出网请求默认直连。

- **默认即可用**：图书 / 电影 / 剧集 / 动画 / 音乐 / 播客 全部走国内可直连的源（豆瓣、微信读书、TapTap、喜马拉雅），不挂代理也能用。
- **需要出境的场景**：IGDB 的授权域名 `id.twitch.tv` 在大陆被墙（其数据域名 `api.igdb.com` 可裸连）。游戏若要从 IGDB 拉取（而非降级走 TapTap），需让**授权请求**出境 —— 但令牌为 `client_credentials` 类型、约 **60 天**有效，因此实际操作是**每两个月借一次网续一次令牌**，续完即回到直连。
- **应用内代理设置（推荐）**：设置 → 数据源 → **代理设置**，填入本机代理软件（如 FlClash）的本地端口 —— HTTP 默认 `127.0.0.1:7890`、SOCKS5 默认 `127.0.0.1:7891`。应用会显式经该端口出墙，绕开「TUN 捕获 Dart 流量后转发层出问题」这条坏路径。
  - 关闭时行为与直连完全一致；开启但代理软件未启动，请求自动退化直连（不会比不开更糟）。
  - `localhost` / `127.0.0.1` 永远直连，自托管服务端不受影响。
- **VPN / TUN 模式**：Android 上 FlClash 等经 VpnService 建 TUN，理论上也能接管流量，但 Dart 流量被 TUN 捕获后转发层偶有兼容问题（开 VPN 后反而全挂）。应用内代理设置是该问题更稳定、更可控的解。
...
界面默认使用内置的 Inter。**Windows** 上可在 **设置 → 外观 → 界面字体** 中改用本机已安装的任意字体；移动端与 Web 端不提供（无本机字体库可读）。

- **字体名取自字体文件本身**，而非文件名 —— 逐个解析 SFNT 的 `name` 表（优先 `id 16` 排版族名，回落 `id 1`），带本地化名（如「微软雅黑」）的会一并显示。
- **`.ttc` / `.otc` 只取第 0 面**，因此一个字体集文件对应一个条目（如 `cambria.ttc` 只列 `Cambria`）。
- **预览零成本**：列表每一行直接用系统族名渲染，无需预先装载；只有真正选中时才读字节并以 `TK:` 前缀的自定义族名注册进引擎，避免与系统同族名混淆。
- **切换即时生效**：选择后通过重建整棵控件树应用，无需重启。
- **持久化在本地**：重启时在首帧之前恢复；若该字体已被卸载，则回落到 Inter 并遗忘这条设置。
- 该设置是**机器本地资源**，不写入 `.xcoll` 配置导出，也不随设备间同步。

## 界面主题

**设置 → 外观 → 主题**，可在三套配色间切换，无需重启。色值由同一套语义 token（`AppPalette`，41 个颜色角色）驱动，切主题即整树重建。

| 主题 | 底色 | 说明 |
|------|------|------|
| **深色** | 近黑 | 默认；长时间阅读不刺眼 |
| **樱花** | 浅粉 | 浅色系，面板与描边以玫瑰色分层 |
| **PS1 复古灰** | 官方灰 `#ADADAD` | 取自 1994 年初代 PlayStation：灰机身配四色标志。蓝、红按色卡原值作填充底（白字对比度 5.30 / 5.04）；黄、绿在中亮灰上作细线时对比度不足（黄仅 0.93，近乎隐形），故按色相压暗后用于图标与文字 |

主题属**外观偏好**：不写入 `.xcoll` 配置导出，也不随设备间同步 —— 每台设备各选各的。
