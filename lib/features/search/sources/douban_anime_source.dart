import 'package:core/models/anime.dart';
import 'package:core/models/data_source.dart';
import 'package:core/models/media_type.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../../../core/api/douban_api.dart';
import '../../../l10n/app_localizations.dart';
import '../models/search_source.dart';

/// Douban's animation catalogue. Douban has no animation subject type — an
/// animated series is a series there and an animated film a film — so this
/// reads the shared film/series pool and keeps the rows tagged `动画`. Every
/// request needs the built-in key pair and the host bans a burst, so it
/// searches only.
class DoubanAnimeSource extends SearchSource {
  @override
  String get id => 'douban_anime';

  @override
  MediaType get outputMediaType => MediaType.anime;

  @override
  DataSource get dataSource => DataSource.douban;

  @override
  String label(S l) => l.mediaTypeAnime;

  @override
  IconData get icon => Icons.animation;

  // The endpoint needs a keyword, and each call spends quota on a ban-prone
  // host, so this source searches only and never browses.
  @override
  bool get supportsBrowse => false;

  // The search endpoint takes no field filters.
  @override
  List<SearchFilter> get filters => const <SearchFilter>[];

  // It takes no sort parameter either.
  @override
  bool get supportsSortDuringSearch => false;

  @override
  List<BrowseSortOption> get sortOptions => const <BrowseSortOption>[
        BrowseSortOption(id: 'relevance', apiValue: ''),
      ];

  @override
  String searchHint(S l) => l.searchHintAnime;

  @override
  Future<BrowseResult> fetch(
    Ref ref, {
    String? query,
    required Map<String, Object?> filterValues,
    required String sortBy,
    required int page,
  }) async {
    final String? trimmed = query?.trim();
    if (trimmed == null || trimmed.isEmpty) {
      return const BrowseResult(items: <Object>[], mediaType: MediaType.anime);
    }

    final DoubanApi api = ref.read(doubanApiProvider);
    final (List<Anime> anime, bool hasMore, int totalPages) =
        await api.searchAnime(query: trimmed, page: page);

    return BrowseResult(
      items: anime,
      mediaType: MediaType.anime,
      hasMore: hasMore,
      totalPages: totalPages,
      currentPage: page,
    );
  }
}
