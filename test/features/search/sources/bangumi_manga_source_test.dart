...
/// Named arguments `BangumiMangaSource` is expected to hand to the API.
...
Manga _manga(int id) => Manga(id: id, title: '漫画 $id');

void main() {
  late BangumiMangaSource source;

  setUp(() {
    source = BangumiMangaSource();
  });

  group('BangumiMangaSource', () {
    test('is a keyless bangumi source serving the manga type', () {
      expect(source.id, 'bangumi_manga');
      expect(source.dataSource, DataSource.bangumi);
      expect(source.outputMediaType, MediaType.manga);
    ...
    group('fetch', () {
      ...
      test('returns the page with the manga media type', () async {
        ...
        expect(result.mediaType, MediaType.manga);
      ...
      test('turns a year selection into a one-year air_date range', () async {
        final Map<String, Object?> args =
            await capture(<String, Object?>{'year': 1997});

        expect(args['airDate'], <String>['>=1997-01-01', '<1998-01-01']);
      });
      ...
      test('forwards the category selection as picked', () async {
        // 漫画 is pinned inside the facade, not here, so the source passes the
        // user's pick through untouched.
        final Map<String, Object?> args = await capture(<String, Object?>{
          'metaTags': <String>['日本', '连载中'],
        });

        expect(args['metaTags'], <String>['日本', '连载中']);
      });
      ...
      test('keeps match while a query is active', () async {
        final Map<String, Object?> args =
            await capture(<String, Object?>{}, query: '海贼王');

        expect(args['query'], '海贼王');
