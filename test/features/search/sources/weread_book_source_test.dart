...
void main() {
  late WeReadBookSource source;

  setUpAll(registerAllFallbacks);

  setUp(() {
    source = WeReadBookSource();
  });

  group('WeReadBookSource', () {
    test('is a keyless book source that only searches', () {
      expect(source.id, 'weread');
      expect(source.dataSource, DataSource.weread);
    ...
    group('fetch', () {
      ...
      setUp(() {
        ...
        final ProviderContainer container = ProviderContainer(
          overrides: <Override>[wereadApiProvider.overrideWithValue(api)],
        );
        ...
        when(() => api.searchBooks(
          ...
          return (<Book>[createTestBook()], true, 2);
        });
      });
      ...
      test('asks even for a single character, which the API answers', () async {
        await run('三');
      ...
      test('propagates the last page as hasMore false', () async {
        when(() => api.searchBooks(
              query: any(named: 'query'),
              page: any(named: 'page'),
            )).thenAnswer((_) async => (<Book>[], false, 2));
