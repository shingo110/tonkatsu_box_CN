...
| 图书 | NeoDB | ✅ 已并入 | 免密钥；**不要求自定义 UA**（与 Bangumi 相反）。无 `query` 是 422、空串是 400 → **不支持空关键词浏览**；分页每页条数不规则（实测 6/11/19），只能信 `pages` 字段；`rating` 已是 0–10，**不可乘 2** |
| 动画 / 影视 | NeoDB 其它类目 | 📋 待实现 | 同一客户端与 `category` 参数即可扩展，缺的是模型映射与筛选器 |
...
| **M3** 影视线 | NeoDB 扩电影 / 剧集（复用现成客户端）优先；若直连豆瓣则须先补 403 退避；另有优酷 / 爱奇艺兜底 | 📋 下一步候选 |
| **M4** 图书线补强 | 微信读书 + 豆瓣 ISBN 直查 | 📋 |
| **M5** 漫画线 | 待确定可行路径后立项 | 🔍 |
| **M6** 发布 | Windows / Android / Web 打包与分发 | 📋 |
